import 'dart:io';
import 'package:bet_kray/Request/Screens/update_request.dart';
...
class DatabaseProvider{
  ...
  Future<List<UserRequest>> getAllRequest() async {
    ...
    List<UserRequest> list =
        response.isNotEmpty ? response.map((c) => UserRequest.fromJson(c)).toList() : []; 
    return list;
  }
...
Future<int> UpdateUserRequest(UserRequest request) async {
    ...
    var result = await db.update("request", request.toMap(), 
